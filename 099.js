...
// 使用前必要修改
// 修改本地私有弹幕库 var arrCommon var arrPoetry；
// 关掉贵族特效 var deityFlag=false 
// 匿名汇报、取消上传 var anonymousFlag=true
// 取消谢礼弹幕判定 取消调用 giftTankBarrage()
// 关闭AI弹幕 取消调用 captureBarrage() 统一调用 bombBarrage()   另一种改法： var selfDanmuFlag = true, mixedFlag = false
// 不区分火力全开发起者 修改function bombBarrage() 统一弹幕内容
// 增加桌面通知组件，增加抽奖结束判定和通知 修改 welfareController()  #被移除
// 金额结合热度推断是否礼物抽奖，减少无效跳转。增加活动房间查询间隔 在 function randomWelfareRequest() 中新增 function waitAndRequest()。新增 var searchWait 查询抽奖查询间隔变量 #被移除
// 关闭上传信息 取消调用 putJumpInfo()和 shareFirePowerInfo()
// 弹幕发送间隔、跳转等待间隔随机数加入负数。新增 function getRandomInterval(), var intervalFactor， 在function randomTime() 和 function waitAndRequest() 中使用 #被移除
// 缩短房间重开等待时间，因应有效跳转增加，总弹幕数增加，减少总跳转次数 var jumpCount = 80;

// 关闭功能
// 1.关闭鱼吧，车队模块
// 读取用户配置 取消调用 userConfig()
// 云弹幕 取消调用 cloudBarrage()
// 车队模块 取消调用 motorcadeOpenTab()
// 更新检查 取消调用 getGFVersionNumber()
// 自动签到 取消调用 autoAssign()
// 清包 sendAllPackageGift()
// 打卡 addFansClickBtn()

// 后端
// 用另一个IP查询筛选，保存至数据库，每次请求时剔除超时房间，返回有效进行中的房间
...
// 已废除功能
//针对刷签到积分的功能
// 增加当日发送弹幕统计 新增function getDailySend()
// 增加当日发送弹幕数全局变量 var sendCount = 0;
// 根据roomId判定初始化时是否读取本地数据 sendCount = getDailySend();
// 根据roomId判定初始化时发送间隔var tmGap
// 新增移除前日弹幕统计 修改function storageOperate()
// 限制每日自动火力弹幕总数 360条，达到则停止循环调用 function firePowerController()
// 根据roomId决定是否采用火力全开鱼丸奖励数量判定 function awardJudge()
// 改手动极速签到，避免调用API签到无弹幕提示。禁用极速签到的自动签到，仅在控制台输出开播状态， 取消调用thunderRoomAssign()  很少用不改了
// 暂时新增在进入房间初始化时自动关注 followRoom() 调用频繁触发验证及block，停用不改了

// 使用更温和的跳转策略 初始化时动态调整var jumpDelay // jumpDelay = getJumpDelay();  （使用金额热度判定后已变相延长并随机跳转请求时间
// 抽奖弹幕延迟发送，且只发送一次 修改welfareController() （暂时移除 弹幕数可能影响中奖率
// 去除中奖发言 取消调用 awardBarrage() (原版已移除
...
// //获取跳转等待时间
// function getJumpDelay() {
//     let lastRequestTime = localStorage.getItem("lastRequestTime");
//     if (lastRequestTime != null) {
//         // 有上次查询值，开始判定
//         let nowTime = (new Date()).getTime();
//         console.info("lastRequestTime:" + lastRequestTime);
//         console.info("nowTime:" + nowTime);
//         console.info("intervalTime:" + ((nowTime - lastRequestTime) / 1000 / 60));
//         if ((nowTime - lastRequestTime) / 1000 / 60 > 20) {
//             //距离上次查询时间超过15分钟，重新设置跳转等待时间
//             jumpDelay = setJumpDelay();
//         } else {
//             //距离上次查询时间没有超过15分钟，直接从本地获取跳转等待时间
//             console.info("jumpDelay:" + localStorage.getItem("jumpDelay"));
//             jumpDelay = localStorage.getItem("jumpDelay");
//         }
//     } else {
//         console.info("本地没有记录上次查询房间数量的时间，开始设置跳转等待时间");
//         jumpDelay = setJumpDelay();
//     }

// }

// // 查询开启抽奖的房间数量 若总数大于等于10，采用更短的跳转等待时间；反正则采用更长的等待时间
// function setJumpDelay() {
//     let jumpDelay;
//     fetch('https://www.douyu.com/japi/weblist/apinc/rec/lottery?num=10&page=1', {
//         method: 'GET',
//         mode: 'cors',
//         cache: 'default',
//         credentials: 'include'
...
//         console.info("获取到的房间数量:" + welFareList.length);
...
//             jumpDelay = 15;
//         }
//         localStorage.setItem("jumpDelay", jumpDelay);
//         localStorage.setItem("lastRequestTime", (new Date()).getTime());
//         return jumpDelay;
//     }).catch(err => {
//         console.info("开启抽奖中数量查询异常");
//     })
// }
...
function roomScript() {
    ...
    //=============================================================================
    var arrCommon = ["6"]; //common barrage, used with clould common barrage
    var arrPoetry = ["6"];//refined barrage,used with clould refined barrage;
    ...
    var fireWait = 80;//if fire power is over,script will wait this value(second) to check whether fire appeared again or redirect；
    var jumpCount = 80;//auto jump times, don't advise to change the value too high; this may conduct your account to be banned speak by douyu;
    var jumpDelay = 3;//default jump delay(unit:second)，user can config this setup by yourself;
    var roomStay = 180;//default room stay time (unit:second)，user can config this setup by yourself;
    var danmuGap = 20000;//default send barrage time gap (ms);
    ...
    var fireFlag = true, deityFlag = false, welfareFlag = true, activeFlag = true, followFlag = false, thankFlag = false, selfDanmuFlag = true, mixedFlag = false, anonymousFlag = true;
    ...
    // 新增 抽奖弹幕已发送标记变量
    var welfareSended = false;
    // 新增 抽奖查询间隔时间
    var searchWait = 10000;
    // 新增 弹幕随机间隔时间因数
    var DanmuIntervalFactor = 5000;
    // 新增 查询随机间隔时间因数
    var SearchIntervalFactor = 2500;
    // 新增低效抽奖忽略清单
    var ignoreRooms = ["隔壁家的小妹妹", "微笑", "DNF枪魂冰子", "小QIMBA", "温柚v最强女打野", "左小亮丶", "沈大盒子", "王师傅精益求精", "主播蛋白i", "错觉老中医"];

    function getRandomInterval(intervalFactor) {
    ...
    function randomTime() {
        ...
        let tmRandom = danmuGap + getRandomInterval(DanmuIntervalFactor);
        console.info("下次弹幕等待时间是：" + tmRandom / 1000 + "秒");
